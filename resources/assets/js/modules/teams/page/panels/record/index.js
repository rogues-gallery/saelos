    ...
    if (team.id === null) {
      return (
        <main className="col main-panel px-3 align-self-center">
          <h2 className="text-muted text-center">Select a team <span className="d-none d-lg-inline">on the left </span>to edit.</h2>
        </main>
        )
    }
          ...
          <div className="card mb-1">
            <ul className={`list-group list-group-flush`}>
			        <li className="list-group-item">
		              ...
		              <div>
		                <input type="text" id="name" name="name" onChange={this._handleInputChange} className="form-control" placeholder="Team Name" value={formState.name} />
		              </div>
	            	</div>
                <div className={`form-group mb-2`}>
                  <label htmlFor="description">Description</label>
                  <div>
                    <textarea id="description" name="description" onChange={this._handleInputChange} className="form-control" defaultValue={formState.description} />
                  </div>
                </div>
	            </li>
			        <li className="list-group-item">
                  ...
                  <Select
                    ...
                    options={formState.users.map(u => ({value: u.id, label: u.name}))}
                    onChange={value => {
                      const event = {
                        target: {
                          name: 'leader_id',
