...
class TagsPartial extends React.Component {
  ...
  _removeTag(tag) {
    ...
    switch (entityType) {
      case "App\\Contact":
        ...
        break;
      case "App\\Company":
        tag.companies = _.filter(
          tag.companies,
          c => c.id !== parseInt(entityId)
        );
        break;
      case "App\\Opportunity":
        tag.opportunities = _.filter(
          tag.opportunities,
          o => o.id !== parseInt(entityId)
        );
        break;
      case "App\\Activity":
        tag.activities = _.filter(
          tag.activities,
          a => a.id !== parseInt(entityId)
        );
        break;
    }
  ...
  render() {
    ...
    const filtered = _.filter(
      allTags,
      at => _.findIndex(tags, t => t.id === at.id) < 0
    );
    const myTags = _.filter(
      allTags,
      at => _.findIndex(tags, t => t.id === at.id) >= 0
    );
        ...
        <small className="ml-3">
          {myTags.map(t => {
            let tagStyle =
              t.color !== "null"
                ? {
                    backgroundColor: convertHex(t.color, 10),
                    borderColor: t.color,
                    color: t.color
                  }
                : {};
                ...
                <div className="add-tag-menu dropdown-menu show mt-1 pt-2">
                  {filtered.map(t => {
                    return (
                      <span
                        key={`entity-${entityId}-tag-${t.id}-${t.color}`}
                        className="dropdown-item px-2 py-2 cursor-pointer"
                        onClick={() => this._tagEntity(t.id)}
                      >
                        <span
                          className="dot mr-2"
