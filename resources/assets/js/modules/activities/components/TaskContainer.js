...
import { getModel, getActionView, isOpen, isMinimized } from '../store/selectors'
      ...
      <div className={`task-container card ${open ? '' : 'd-none'}`}>
        <ErrorBoundary>
          <div className="card-header bg-dark-grey py-3">
            <h5 className="m-0 text-light" onClick={(e) => minimized ? this._toggleMinimize(e) : null}>
              ...
              <button type="button" className="close" aria-label="Minimize" onClick={this._toggleMinimize}>
                <span aria-hidden="true" className="text-light">&nbsp;_&nbsp;</span>
              </button>
            </h5>
          </div>
